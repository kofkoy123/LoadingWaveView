...
public class LoadingWaveView extends View {
    ...
    public LoadingWaveView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public LoadingWaveView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(AttributeSet attrs, int defStyle) {
        final TypedArray a = getContext().obtainStyledAttributes(
                attrs, R.styleable.LoadingWaveView, defStyle, 0);
        mWaveColor = a.getColor(
                R.styleable.LoadingWaveView_waveColor,
                mWaveColor);
        mBaseLineColor = a.getColor(
                R.styleable.LoadingWaveView_baselineColor,
                mBaseLineColor);

        waveStrokeWidth = a.getDimension(
                R.styleable.LoadingWaveView_waveStokeWidth,
                waveStrokeWidth);

        max = (short) a.getInt(R.styleable.LoadingWaveView_maxValue, max);
        invalidateTime = a.getInt(R.styleable.LoadingWaveView_invalidateTime, invalidateTime);

        space = a.getDimension(R.styleable.LoadingWaveView_space, space);
        a.recycle();
        initPainters();
